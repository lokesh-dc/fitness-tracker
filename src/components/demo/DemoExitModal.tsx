...
export function DemoExitModal() {
	...
	useEffect(() => {
		...
		const handleMouseLeave = (e: MouseEvent) => {
			// Trigger if mouse leaves the top of the window (moving towards tabs/address bar)
			if (e.clientY <= 0) {
				showModal();
			}
		};
		...
		// Mobile/Desktop Back Button Trigger
		const handlePopState = (e: PopStateEvent) => {
			if (!hasShown) {
				// Prevent the actual back navigation by pushing state back
				window.history.pushState(null, "", window.location.href);
				showModal();
			}
		};

		// Push initial state to catch the first back button click
		if (!hasShown) {
			window.history.pushState(null, "", window.location.href);
		}
							...
							<div className="flex flex-col items-center text-center space-y-6">
								...
								<div className="space-y-2">
									...
									<p className="text-sm font-medium text-foreground/60 leading-relaxed">
										Your demo session will reset in 24 hours. Create a free
										account to keep your data forever.
									</p>
								</div>

...
export default function OnboardingFlow({ initialData }: OnboardingFlowProps) {
	...
	useEffect(() => {
		...
		if (step) {
			const startStep = parseInt(step) - 1;
	...
	const progressColor = currentStep === 4 ? "bg-emerald-500" : "bg-orange-500";
	const progressBaseColor =
		currentStep === 4 ? "bg-emerald-500/50" : "bg-orange-500/50";
	...
	return (
		<div className="min-h-[100dvh] bg-background flex flex-col items-center">
			...
			{/* Mobile Header */}
			<div className="md:hidden pt-8 pb-4 flex items-center justify-between w-full px-6 animate-in fade-in slide-in-from-top-4 duration-1000">
				<div className="flex items-center gap-3">
					...
					<span className="text-[18px] font-black uppercase tracking-[0.2em] text-foreground italic">
						FitTrack
					</span>
				</div>
				<ThemeToggle />
					...
					<div className="w-full md:max-w-[520px]">
						<AnimatePresence mode="wait" custom={direction}>
							<motion.div
								...
								className="w-full">
								<GlassCard className="space-y-12 border-foreground/5 md:p-0 md:bg-transparent md:backdrop-blur-none md:shadow-none md:border-none pb-12">
									{currentStep === 0 && (
											...
											<div className="space-y-8">
												<div className="space-y-6">
													<div className="flex justify-between items-center px-2">
														...
														<div className="bg-foreground/5 p-1 rounded-xl flex gap-1">
												...
												<div className="space-y-6">
													...
													<GlassSlider
														...
														value={
															heightUnit === "cm" ? height : cmToFt(height)
														}
														onChange={(v) =>
										...
										<div className="space-y-10">
											...
											<div className="space-y-8">
												<GlassSlider
													...
													unit={weightUnit}
												/>
												<div className="text-left py-4 px-6 bg-foreground/5 rounded-2xl border border-foreground/5">
													...
													<button
														...
														className={cn(
															...
															targetTimeline === t
																...
																: "border-foreground/5 bg-foreground/5 text-foreground/40 hover:bg-foreground/10",
														)}>
														<div className="flex flex-col items-start">
															...
															<span
																className={cn(
																	"text-[10px] font-bold uppercase tracking-widest group-hover:text-foreground/50 transition-colors",
																	targetTimeline === t
																		? "text-white/60"
																		: "text-foreground/30",
																)}>
									...
									{currentStep === 4 && (
										<div className="space-y-10">
											...
											/>
											<div className="space-y-3 bg-foreground/5 rounded-3xl p-8 border border-foreground/5">
												<SummaryRow
													label="Current Weight"
										...
										<div
											className={cn(
												...
												idx < currentStep
													...
													: idx === currentStep
														? "bg-background border-orange-500 text-orange-500 scale-100"
														: "bg-background border-foreground/5 text-foreground/10",
											)}>
											{idx < currentStep ? (
					...
					<div className="w-full max-w-6xl mx-auto flex justify-center px-4">
						<div className="flex-1 flex flex-col md:flex-row gap-6 md:gap-32 md:items-start md:justify-center">
							<div className="w-full md:max-w-[520px] pointer-events-auto relative">
								...
								<div className="absolute -top-40 left-0 right-0 h-40 pointer-events-none bg-gradient-to-t from-background via-background/95 to-transparent" />
...
function OnboardingHeader({
	...
	return (
		<div className="text-left space-y-2">
			<h1 className="text-2xl md:text-xl font-black text-foreground uppercase tracking-tight leading-none">
				{title}
			</h1>
	...
	return (
		<div className="space-y-6">
			<button
				...
				className={cn(
					"w-full h-16 md:h-14 rounded-2xl font-black text-lg md:text-sm uppercase tracking-widest shadow-xl active:scale-[0.98] transition-all disabled:opacity-50 text-white",
					isFinish
			...
			<div className="flex justify-center items-center gap-8 px-2">
				{onBack && (
					<button
						onClick={onBack}
						className="text-foreground/20 text-[10px] font-black uppercase tracking-[0.2em] hover:text-foreground/40 transition-colors">
						Go Back
					</button>
				)}
				<button
					onClick={onSkip}
					className="text-foreground/30 text-[10px] font-black uppercase tracking-[0.2em] hover:text-foreground transition-colors">
					Skip for now
				</button>
			</div>
		</div>
...
function SummaryRow({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex justify-between items-center py-3 border-b border-foreground/5 last:border-0">
			<span className="text-foreground/30 text-[9px] font-black uppercase tracking-widest">
				{label}
			</span>
			<span className="text-foreground font-bold text-xs md:text-[11px] tracking-tight">
				{value || "—"}
			</span>
		</div>
	);

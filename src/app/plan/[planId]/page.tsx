...
import { Calendar, ChevronLeft, LayoutGrid, Trophy } from "lucide-react";
import { GlassCard } from "@/components/ui/GlassCard";
import { getPlanDetails, getPlanSidebarData } from "@/app/actions/plan";
...
import {
	ActivePlanSidebar,
	ActivePlanMobileStrip,
} from "@/components/sidebar/ActivePlanSidebar";
...
export default async function PlanDetailPage({
	...
	const [data, sidebarData] = await Promise.all([
		...
		userId ? getPlanSidebarData(planId, userId) : Promise.resolve(null),
	]);
	...
	const baseWeek = templates
		.filter(
			(t: WorkoutTemplate) => t.weekNumber === 1 && t.exercises.length > 0,
		)
		.sort(
			(a: WorkoutTemplate, b: WorkoutTemplate) => a.dayOfWeek - b.dayOfWeek,
		);
		...
		<div className="flex flex-col pb-32">
			...
			<main className="px-6 w-full">
				<PageWithSidebar
					sidebar={
						<ActivePlanSidebar
							progressData={sidebarData?.progressData || null}
							strengthData={sidebarData?.strengthData || null}
							volumeData={sidebarData?.volumeData || null}
							bodyweightData={sidebarData?.bodyweightData || null}
						/>
					}
					mobileWidgets={
						<ActivePlanMobileStrip

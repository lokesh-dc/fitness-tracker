    ...
    <div className="flex flex-col min-h-screen">
      <header className="px-6 py-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Insights & Progress</p>
        </div>
        <ThemeToggle />
      </header>
      ...
      <main className="flex-1 px-6 space-y-8 max-w-4xl mx-auto w-full pb-12">
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-end">
            <h2 className="text-lg font-bold text-foreground tracking-tight">Personal Records</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {prs.map((pr, idx) => (
              <GlassCard key={idx} className="relative overflow-hidden group">
                ...
                <p className="text-[10px] font-black text-foreground/40 uppercase tracking-widest mb-1">{pr.name}</p>
                <div className="flex items-baseline space-x-1 mb-2">
                  <span className="text-2xl font-black text-foreground">{pr.weight}</span>
                  <span className="text-[10px] font-bold text-foreground/40 uppercase">KG</span>
                </div>
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-bold text-foreground tracking-tight">Recent History</h2>
             ...
             {[1, 2, 3].map((i) => (
               <GlassCard key={i} className="flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-foreground">Strength Session</h3>
                      <p className="text-[10px] text-foreground/40 font-medium uppercase tracking-wider">March {10-i}, 2024 • 65 min</p>
                    </div>
                  </div>
